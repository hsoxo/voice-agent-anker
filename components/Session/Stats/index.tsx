...
const StatsTile = ({
  ...
  return (
    <div className="text-sm bg-white border border-primary-200 rounded-md">
      ...
      <div className="px-3">
        <Sparklines data={data.timeseries} limit={20} height={80} svgHeight={80}>
      ...
      <footer className="border-t border-primary-200 text-[11px] font-mono flex flex-row justify-between px-3 py-2">
        <div className="uppercase font-bold flex gap-1">
          H:<span className="font-normal">{data.high?.toFixed(multiplier)}<sub>{sub}</sub></span>
        </div>
        <div className="uppercase font-bold flex gap-1">
          M:<span className="font-normal">{data.median?.toFixed(multiplier)}<sub>{sub}</sub></span>
        </div>
        <div className="uppercase font-bold flex gap-1">
          L:<span className="font-normal">{data.low?.toFixed(multiplier)}<sub>{sub}</sub></span>
        </div>
      </footer>
    </div>
  );
